...
highlighter = null
{resourcePath} = via.getLoadSettings()
...
exports.toDOMFragment = (text='', filePath, grammar, callback) ->
  render text, filePath, (error, html) ->
    ...
    convertCodeBlocksToViaEditors(domFragment, defaultCodeLanguage)
...
render = (text, filePath, callback) ->
  ...
  options =
    sanitize: false
    breaks: via.config.get('markdown-preview.breakOnSingleNewline')
  ...
  roaster text, options, (error, html) ->
    ...
    html = createDOMPurify().sanitize(html, {ALLOW_UNKNOWN_PROTOCOLS: via.config.get('markdown-preview.allowUnsafeProtocols')})
    html = resolveImagePaths(html, filePath)
    callback(null, html.trim())

resolveImagePaths = (html, filePath) ->
  [rootDirectory] = via.project.relativizePath(filePath)
  ...
  for img in o.querySelectorAll('img')
    ...
    if src = img.getAttribute('src')
      continue if src.match(/^(https?|via):\/\//)
...
convertCodeBlocksToViaEditors = (domFragment, defaultLanguage='text') ->
  if fontFamily = via.config.get('editor.fontFamily')
  ...
  for preElement in domFragment.querySelectorAll('pre')
    ...
    editorElement = document.createElement('via-text-editor')
    ...
    if grammar = via.grammars.grammarForScopeName(scopeForFenceName(fenceName))
...
tokenizeCodeBlocks = (html, defaultLanguage='text') ->
  ...
  for preElement in o.querySelectorAll("pre")
    ...
    highlighter ?= new Highlights(registry: via.grammars, scopePrefix: 'syntax--')
